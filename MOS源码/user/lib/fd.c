...
int fd_alloc(struct Fd **fd) {  //不申请物理内存！
	u_int va;
	u_int fdno;
...
char* getcwd() {
	//debugf("%s", env->env_workingpath);
	return env->env_workingpath;
}
...
int chdir(char *path) { //后续再添加 .和..
	char *nowpath = env->env_workingpath;
	if (path[0] == '/') {
		parsepath(path);
		return chdir1(finalpath);
	} else { //打头不是/
		char newpath[1024];
		memset(newpath, 0, 1024); //必要的
		strcpy(newpath, nowpath);
	 ...
	 //先字符串拼接成最终形态 再进到文件服务进程完成合法性判断 111
}

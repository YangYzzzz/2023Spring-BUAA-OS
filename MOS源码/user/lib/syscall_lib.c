...
char * syscall_get_env_var(u_int envid, const char *name) {
	return msyscall(SYS_get_env_var, envid, name);
}
void syscall_yield(void) {
	msyscall(SYS_yield);
}
void syscall_set_shid(u_int envid) {
	msyscall(SYS_set_shid, envid);
}
int syscall_set_env_var(u_int envid, int op, const char *name, const char *value) {
	return msyscall(SYS_set_env_var, envid, op, name, value);
}
int syscall_env_destroy(u_int envid) {
	return msyscall(SYS_env_destroy, envid);
}
int syscall_change_workingpath(u_int envid, const char *newpath) {
	return msyscall(SYS_change_workingpath, envid, newpath);
}

...
int file_mkdir(char *path, int p) {
	int r;
	if ((r = fsipc_mkdir(path, p)) < 0) {
		return r;
	}
	return 0;
}
...
int file_touch(char *path, int p) {
	int r;
	if ((r = fsipc_touch(path, p)) < 0) {
		return r;
	}
	return 0;
}
...
int file_copy(char *name, char *path, int n) {
	int r;
        if ((r = fsipc_copy(name, path, n)) < 0) {
              return r;
        }
        return 0;
}
int file_move(char *name, char *path, int n) {
	int r;
	if ((r = fsipc_move(name, path, n)) < 0) {
		return r;
	}
	return 0;
}
int get_file_size(int fdnum) {
	int r;
	struct Fd *fd;
	if ((r = fd_lookup(fdnum, &fd)) < 0) {
		debugf(" open error!\n");
...
static int file_write(struct Fd *fd, const void *buf, u_int n, u_int offset) {
	...
	//debugf("1");
	f = (struct Filefd *)fd;
	if (f->f_file.f_type == FTYPE_DIR) {
		debugf("dir writing, error!\n");
		exit();
	}
...
int myremove(const char *path, int force, int recursive, int dir) {
	return fsipc_remove(path, force, recursive, dir);
}
int remove(const char *path) {
	...
	return fsipc_remove(path, 0, 0, 0);
}

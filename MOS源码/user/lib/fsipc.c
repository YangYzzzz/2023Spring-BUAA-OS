...
int fsipc_copy(const char *name, const char *path, int n) {
	if (strlen(name) >= MAXPATHLEN || strlen(path) >= MAXPATHLEN) {
		return -E_BAD_PATH;
	}
...
int fsipc_move(const char *name, const char *path, int n) {
	if (strlen(name) >= MAXPATHLEN || strlen(path) >= MAXPATHLEN) {
		return -E_BAD_PATH;
	}
	struct Fsreq_move *req;
	req = (struct Fsreq_copy *)fsipcbuf;
	strcpy((char *)req->req_name, name);
	strcpy((char *)req->req_path, path);
	req->req_n = n;
	return fsipc(FSREQ_MOVE, req, 0, 0);
}
int fsipc_mkdir(const char *path, int p) {
	if (strlen(path) >= MAXPATHLEN) {
		return -E_BAD_PATH;
	}
...
int fsipc_touch(const char *path, int p) {
	if (strlen(path) >= MAXPATHLEN) {
		return -E_BAD_PATH;
	}
	struct Fsreq_touch *req;
	req = (struct Fsreq_touch *)fsipcbuf;
	strcpy((char *)req->req_path, path);
	req->req_p = p;
	return fsipc(FSREQ_TOUCH, req, 0, 0);
}
// Overview:
...
int fsipc_remove(const char *path, int force, int recursive, int dir) {

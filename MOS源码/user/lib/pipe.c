...
int pipe(int pfd[2]) {
	...
	fd0->fd_dev_id = devpipe.dev_id; //文件描述符，不需要文件控制块
...
static int _pipe_is_closed(struct Fd *fd, struct Pipe *p) {
	...
	int fd_ref, pipe_ref, runs; //runs是什么意思 存疑
...
static int pipe_read(struct Fd *fd, void *vbuf, u_int n, u_int offset) {
	...
	p = fd2data(fd); //得到管道的地址
	//debugf("read fdnum : %d\n", fd2num(fd));
	while (1) { //往前读到头
		if (p->p_rpos < p->p_wpos) {
			*rbuf = p->p_buf[p->p_rpos % BY2PIPE];
			//debugf(" read wpos : %d, rpos : %d, char : %c !!!!\n",p->p_wpos, p->p_rpos,  *rbuf);
			i++;
			rbuf++;
			p->p_rpos++;
			if (i == n) {
...
static int pipe_write(struct Fd *fd, const void *vbuf, u_int n, u_int offset) {
	...
	p = fd2data(fd);
	//debugf("write fdnum : %d\n", fd2num(fd));
	while (1) {
		if (p->p_wpos - p->p_rpos < BY2PIPE) {
                        p->p_buf[p->p_wpos % BY2PIPE] = *wbuf;
			//debugf(" write wpos : %d, rpos : %d, char : %c !!!!\n",p->p_wpos, p->p_rpos,  *wbuf);
                        wbuf++;
                        p->p_wpos++;
			i++;
			if (i == n) {
				return n;
                	...
                	if (_pipe_is_closed(fd, p)) { //管道一端关闭，则另一端在完成任务后也该关闭
                        	return i;
                	}
                        syscall_yield();
		}

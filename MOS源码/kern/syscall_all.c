...
int sys_change_workingpath(u_int envid, char *newpath) { //记录在进程中
	struct Env *env;
	if (envid2env(envid, &env, 1) == -E_BAD_ENV) {
                return -E_BAD_ENV;
        }
	//printk("sys : %s\n", newpath);
	strcpy(env->env_workingpath, newpath);
	return 0;
}
/* Overview:
 ...
 *
 * Pre-Condition:
 * 	`s` is base address of the string, and `num` is length of the st`ring.
 */
...
int sys_mem_map(u_int srcid, u_int srcva, u_int dstid, u_int dstva, u_int perm) {
	...
	if (envid2env(srcid, &srcenv, 1) == -E_BAD_ENV) {
		return -E_BAD_ENV;
	}
 ...
 */
u_int shid = 0;
void sys_set_shid(u_int envid) {
	//printk("envid : %d, ", envid);
	struct Env *env;
...
struct Environment environ[MAXVAR];
struct Environment* alloc_envir() {
       int i;
       for (i = 0; i < 100; i++) {
             if (environ[i].name[0] == 0) {
                      return environ + i;
             }
       }
       return NULL;
...
struct Environment* isexist(char *name, int shid) {
     int i;
     for (i = 0; i < MAXVAR; i++) {
             if (strcmp(environ[i].name, name) == 0 && (environ[i].shid == shid || environ[i].shid == 0)) {
                     return environ + i;
             }
     }
     return NULL;
}
void set_envir(int isread, int shid, char *name, char *value) {
	...
	if (op == 0) {
		for (int i = 0; i < MAXVAR; i++) {
			if (environ[i].name[0] != 0 && (environ[i].shid == 0 || environ[i].shid == shid)) {
				if (environ[i].shid == 0) {
					printk("\033[34mGLOBAL NAME \033[0m: %s, \033[34mVALUE \033[0m: %s", environ[i].name, environ[i].value);
				} else if (environ[i].shid == shid) {
					printk("\033[36mLOCAL NAME \033[0m: %s, \033[94mVALUE \033[0m: %s", environ[i].name, environ[i].value);
				}
				if (environ[i].isread == 1) {
					printk("\033[93m OnlyRead\033[0m\n");
				} else {
					printk("\n");
				}
			}
...
int sys_exofork(void) {
	...
	//memcpy((void *)&(e->env_tf), (void *)&(curenv->env_tf), sizeof(curenv->env_tf));
	/* Step 3: Set the new env's 'env_tf.regs[2]' to 0 to indicate the return value in child. */
	/* Exercise 4.9: Your code here. (3/4) */
	//e->env_tf.cp0_epc = e->env_tf.cp0_epc + 4;
	e->env_tf.regs[2] = 0;
	/* Step 4: Set up the new env's 'env_status' and 'env_pri'.  */
	/* Exercise 4.9: Your code here. (4/4) */
	e->env_status = ENV_NOT_RUNNABLE;
	e->env_pri = curenv->env_pri;
	e->env_shid = curenv->env_shid;
	//printk("child shid : %d\n", e->env_shid);
	memcpy(e->env_workingpath, curenv->env_workingpath, sizeof curenv->env_workingpath);
	return e->env_id;
}
...
int sys_ipc_recv(u_int dstva) {
	...
	((struct Trapframe *)KSTACKTOP - 1)->regs[2] = 0; //这步什么意思
	schedule(1);
}
...
int is_illegal_pa_range(u_int pa, u_int len) {
	return !((pa >= 0x10000000 && pa + len <= 0x10000000 + 0x20) || (pa >= 0x13000000 && pa + len <= 0x13000000 + 0x4200) || (pa >= 0x15000000 && pa + len <= 0x15000000 + 0x200));
}
...
void *syscall_table[MAX_SYSNO] = {
    ...
    [SYS_set_shid] = sys_set_shid,
    [SYS_set_env_var] = sys_set_env_var,
    [SYS_get_env_var] = sys_get_env_var,
    [SYS_change_workingpath] = sys_change_workingpath,
};
...
void do_syscall(struct Trapframe *tf) {
	...
	int sysno = tf->regs[4];  //a0 第一个参数 系统调用号
	if (sysno < 0 || sysno >= MAX_SYSNO) {
		tf->regs[2] = -E_NO_SYS;
		return;
	}
	//printk("syscall num : %d ",sysno );
	/* Step 1: Add the EPC in 'tf' by a word (size of an instruction). */
	/* Exercise 4.2: Your code here. (1/4) */
	tf->cp0_epc = tf->cp0_epc + 4;  // 这步的意思是返回地址跳过当前syscall的这一条指令 即已经执行过去了

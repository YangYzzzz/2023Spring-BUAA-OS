...
static void map_segment(Pde *pgdir, u_int asid, u_long pa, u_long va, u_int size, u_int perm) {
	...
	for (int i = 0; i < size; i += BY2PG) {
		...
		struct Page *page = pa2page(pa + i);
		page_insert(pgdir, asid, page, va + i, perm);
	}
}
...
int envid2env(u_int envid, struct Env **penv, int checkperm) {
	...
	if (checkperm != 0) {
		if (e->env_id != curenv->env_id &&  e->env_parent_id != curenv->env_id) {
			printk(" e->envid : %d, e->env_parent_id : %d, curenv->env_id : %d...", e->env_id, e->env_parent_id, curenv->env_id);
			return -E_BAD_ENV;
		}
	}
...
static void load_icode(struct Env *e, const void *binary, size_t size) {
	...
	e->env_tf.cp0_epc = ehdr->e_entry; //存储入口的虚拟地址
	//printk(" id: %d, epc: %d ", e->env_id, e->env_tf.cp0_epc);
}
...
void env_run(struct Env *e) {
	...
	//进程间的转换 保存即将结束的进程的上下文 并将即将进行的上下文装载进寄存器中
	/* Step 2: Change 'curenv' to 'e'. */
	curenv = e; //进程控制块
	curenv->env_runs++; // lab6
	//printk(" env_run curenv_id : %d..", curenv->env_id);

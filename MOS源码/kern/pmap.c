...
void mips_detect_memory() {
	...
	memsize = *(volatile u_int *)(KSEG1 | DEV_MP_ADDRESS | DEV_MP_MEMORY); // 获取最大地址
...
void page_init(void) {
	...
	//printk("%d", npage - PADDR(freemem)/BY2PG);
	for(i=PADDR(freemem)/BY2PG; i<npage; i++) {
		newPage = pages + i;
		newPage->pp_ref = 0;
		LIST_INSERT_HEAD(&page_free_list, newPage, pp_link);
	}
}
...
void page_free(struct Page *pp) {
	assert(pp->pp_ref == 0);//断言 不满足条件就返回
...
static int pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte) {
	...
	if ((*pgdir_entryp & PTE_V)  == 0 && create == 1) {   //valid = 0 不合法
              ...
              *ppte = PTX(va) + (Pte *)page2kva(pp);
               ...
               *ppte = PTX(va) + (Pte *)page2kva(pages + PPN(*pgdir_entryp)); //第xx个物理页面的   址
               //*ppte = PTX(va) + (Pte *)KADDR(PTE_ADDR(*pgdir_entryp));
      }
...
struct Page *page_lookup(Pde *pgdir, u_long va, Pte **ppte) { //pte两级页表项
	struct Page *pp;
	Pte *pte;

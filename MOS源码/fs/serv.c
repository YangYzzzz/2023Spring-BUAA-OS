...
int open_alloc(struct Open **o) {
	...
	for (i = 0; i < MAXOPEN; i++) {  // 若是和物理页映射则至少映射了两次 文件服务进程一次 用户进程一次 这才算有效的打开表项；若小于两次 则可以重新申请
...
void serve_mkdir(u_int envid, struct Fsreq_mkdir *rq) {
	int r = mkdir(rq->req_path, rq->req_p);
	if (r < 0) {
...
void serve_copy(u_int envid, struct Fsreq_copy *rq) {
	int r = copy(rq->req_name, rq->req_path, rq->req_n);
	if (r < 0) {
		ipc_send(envid, r, 0, 0);
	}
	ipc_send(envid, 0, 0, 0);
}
void serve_move(u_int envid, struct Fsreq_move *rq) {
	int r = move(rq->req_name, rq->req_path, rq->req_n);
	if (r < 0) {
		ipc_send(envid, r, 0, 0);
	}
	ipc_send(envid, 0, 0, 0);
}
void serve_touch(u_int envid, struct Fsreq_touch *rq) {
	int r = touch(rq->req_path, rq->req_p);
	if (r < 0) {
		ipc_send(envid, r, 0, 0);
		return;
	}
...
void serve_remove(u_int envid, struct Fsreq_remove *rq) {
	...
	r = file_remove(rq->req_path, rq->req_f, rq->req_r, rq->req_dir); 
...
void serve(void) {
	...
	for (;;) {
		...
		case FSREQ_SYNC:
			...
			break;
		case FSREQ_MKDIR:
			serve_mkdir(whom, (struct Fsreq_mkdir *)REQVA);
			break;
		case FSREQ_TOUCH:
			serve_touch(whom, (struct Fsreq_touch *)REQVA);
			break;
		case FSREQ_COPY:
			serve_copy(whom, (struct Fsreq_copy *)REQVA);
			break;
		case FSREQ_MOVE:
			serve_move(whom, (struct Fsreq_move *)REQVA);
			break;
		default:

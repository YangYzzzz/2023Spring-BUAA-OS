...
void ide_read(u_int diskno, u_int secno, void *dst, u_int nsecs) {
	...
	for (u_int off = 0; begin + off < end; off += BY2SECT) {
		...
		re = begin + off;
		panic_on(syscall_write_dev(&re, DEV_DISK_ADDRESS + DEV_DISK_OFFSET, 4));
		re = DEV_DISK_OPERATION_READ;
		panic_on(syscall_write_dev(&re, DEV_DISK_ADDRESS + DEV_DISK_START_OPERATION, 4));
